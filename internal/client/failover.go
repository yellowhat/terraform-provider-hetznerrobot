...
func (c *HetznerRobotClient) FetchFailover(ctx context.Context, ip string) (Failover, error) {
	resp, err := c.DoRequest(ctx, "GET", "/failover/"+ip, nil, "")
	if err != nil {
		return Failover{}, fmt.Errorf("FetchFailover request error: %w", err)
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		return Failover{}, fmt.Errorf(
			"FetchFailover %s: status %d, body %s",
			ip, resp.StatusCode, body,
		)
	}
	...
	if err != nil {
		return Failover{}, fmt.Errorf("FetchFailover decode error: %w", err)
	}
...
func (c *HetznerRobotClient) DeleteFailover(ctx context.Context, ip string) error {
	resp, err := c.DoRequest(ctx, "DELETE", "/failover/"+ip, nil, "")
	if err != nil {
